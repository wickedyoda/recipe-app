...
def _extract_recipe_from_text(text: str) -> dict:
    ...
    for line in normalized:
        lower = line.lower()
        if lower.startswith("instruction") or lower.startswith("step") or lower.startswith("how to") or lower.startswith("directions") or lower.startswith("method"):
            section = "instructions"
            continue
        if section == "ingredients":
            if _is_ingredient_like(line):
                ingredients.append(line)
            elif len(ingredients) >= 2 and not lower.startswith("ingredient") and not lower.startswith("you'll need") and not lower.startswith("what you need"):
                section = "instructions"
                instructions.append(line)
            elif lower.startswith("ingredient") or lower.startswith("you'll need") or lower.startswith("what you need"):
                continue
            else:
                ingredients.append(line)
